...
class CostumeTable
{
     ...
     *
     * @param boolean $usePaginator Indique si on veut la liste de réponse sous forme d'un itérable paginé
     * @param string $order Clause order de la requête select
     *
    ...
    /**
     * Retourne les n derniers costumes créés
     *
     * @param int $count Nombre de derniers costumes à récupérer ; strictement positif.
     * @param boolean $usePaginator Indique si on veut la liste de réponse sous forme d'un itérable paginé
     *
     * @throws \Exception
     *
     * @return Costume[] Liste des costumes (sous forme d'un itérable)
     */
    public function getLastCreatedCostumes($count = 10, $usePaginator = false)
    {
        $this->populateCaches();

        if ((int)$count <= 0) {
            throw new \Exception("Invalid \$count value ($count) for ".__METHOD__);
        }

        $select = $this->tableGateway->getSql()->select();
        $select->order("creation_ts DESC");
